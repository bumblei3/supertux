...
BicyclePlatformChild::update(float dt_sec)
{
  ...
  Vector const dest = m_parent.m_center + Vector(cosf(angle), sinf(angle)) * m_parent.m_radius - (m_col.m_bbox.get_size().as_vector() * 0.5f);
  Vector const movement = dest - get_pos();
